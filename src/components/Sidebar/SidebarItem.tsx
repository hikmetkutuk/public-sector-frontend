...
import { SidebarItemProps, SidebarItemType } from '@/types';
...
const SidebarItem = ({ item, pageName, setPageName }: SidebarItemProps) => {
  ...
  useEffect(() => {
    setClientPageName(pageName);
  }, [pageName]);

  const handleClick = () => {
    const updatedPageName =
      clientPageName !== item.label.toLowerCase() ? item.label.toLowerCase() : '';
  ...
  const isActive = (item: SidebarItemType): boolean => {
    ...
    if (item.children) {
      return item.children.some((child: SidebarItemType) => isActive(child));
    }
    return false;
  };
        ...
        {item.children && (
          <svg
            className={clsx(
              ...
              clientPageName === item.label.toLowerCase() ? 'rotate-180' : ''
            )}
            width="20"
            height="20"
            viewBox="0 0 20 20"
      ...
      {item.children && (
        <div
          className={`translate transform overflow-hidden ${
            clientPageName !== item.label.toLowerCase() ? 'hidden' : ''
          }`}
        >
